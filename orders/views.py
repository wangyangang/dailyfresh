        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):
                    ...
                    # 返回受影响的行数
                    res = Goods_SKU.objects.filter(id=sku_id, stock=orgin_stock).update(stock=new_stock, sales=new_sales)
                    if res == 0:
                        if i == 2:
                            # 尝试的第3次
                            transaction.savepoint_rollback(save_id)
...
class OrderDeleteView(View):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse({'res': 4, 'errmsg': '删除错误:%s' % e})
        # ret = order.delete()
...
class OrderPayView(View):
    def post(self, request):
        ...
        alipay = AliPay(
            ...
            app_private_key_path=os.path.join(settings.BASE_DIR, settings.APP_PRIVATE_KEY_PATH),
            alipay_public_key_path=os.path.join(settings.BASE_DIR, settings.ALIPAY_PUBLIC_KEY_PATH), # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
...
class CheckPayView(View):
    ...
    def post(self, request):
        ...
        alipay = AliPay(
            ...
            app_notify_url=None,  # 默认回调url
            app_private_key_path=os.path.join(settings.BASE_DIR, settings.APP_PRIVATE_KEY_PATH),
            alipay_public_key_path=os.path.join(settings.BASE_DIR, settings.ALIPAY_PUBLIC_KEY_PATH),
